...
type RelationshipState = import('./-state').RelationshipState;
...
type UpgradedMeta = import('./-edge-definition').UpgradedMeta;
...
export class Membership {
  ...
  pushState(values: StableRecordIdentifier[]): Diff<StableRecordIdentifier> {
    ...
    this._size = values.length;
    this.data = makeArray(values);
    let newDict = (this._dict = Object.create(null));
...
export class TrackedMembership {
  ...
  remove(value: StableRecordIdentifier) {
    ...
    if (DEBUG) {
      ...
      removals.push(value);
      this._removals = makeArray(removals);
      let dict = (this._removedDict = this._removedDict || Object.create(null));
      dict[id] = true;
      this.isDirty = true;
      return;
    }
