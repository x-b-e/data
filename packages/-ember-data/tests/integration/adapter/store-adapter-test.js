...
module('integration/adapter/store-adapter - DS.Store and DS.Adapter integration test', function (hooks) {
  ...
  test('calling store.didSaveRecord can provide an optional hash', async function (assert) {
    ...
    let tom2 = records2.tom;
    let yehuda2 = records2.yehuda;
    assert.false(tom2.hasDirtyAttributes, 'the record should not be dirty');
    assert.strictEqual(tom2.updatedAt, 'now', 'the hash was updated');
    assert.false(yehuda2.hasDirtyAttributes, 'the record should not be dirty');
    assert.strictEqual(yehuda2.updatedAt, 'now!', 'the hash was updated');
  });
